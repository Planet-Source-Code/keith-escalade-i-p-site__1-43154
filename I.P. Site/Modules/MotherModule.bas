...
End Sub
Public Sub WriteTxtFile(PrintText$, TheDestination$, Append As Boolean)
Rem Saves text to a text file
On Error GoTo ReportError
If Append = True Then
Dim X%
X% = FreeFile
...
Dim I%
I% = FreeFile
Open TheDestination$ For Output As #I
Print #I, PrintText$
Close #I
End If
Exit Sub
...
MsgBox Error, vbCritical
End Sub
Function OpenTxtFile(TheDestination$, ShowError As Boolean)
On Error GoTo ReportError
Dim X%
X = FreeFile
Open TheDestination$ For Input As #X
Input #X, sText$
OpenTxtFile = sText$
Close #X
Exit Function
ReportError:
If ShowError = True Then: MsgBox Error, vbCritical
End Function
...
End Function
Sub LaunchUrl(WhichUrl As String)
Rem Opens a webpage
On Error GoTo DieStupidError
Shell ("C:\Program Files\Internet Explorer\IEXPLORE.EXE " + WhichUrl), vbMaximizedFocus
Exit Sub
DieStupidError:
Exit Sub
End Sub
Function AMorPM()
Rem Finds out if the current time is in AM or PM
If Right(Time, 2) = "PM" Then AMorPM = "PM": Exit Function
If Right(Time, 2) = "AM" Then AMorPM = "AM": Exit Function
End Function
Function FilterTime()
Rem Returns the current time with no colons and AM or PM
Dim TheTime$
TheTime = Left(Time, Len(Time) - 3)
TheTime = Replace(TheTime, ":", "")
FilterTime = TheTime
End Function
Function TheHour()
Rem Returns the hour in the current time
If Mid(Time, 2, 1) = ":" Then TheHour = Left(Time, 1): Exit Function
TheHour = Left(Time, 2)
End Function
Function TheMinute()
Rem Returns the minute in the current time
Dim StartColon%
Dim EndColon%
Dim EndProduct$
StartColon% = InStr(1, Time, ":")
EndColon% = InStr(StartColon% + 1, Time, ":")
EndProduct$ = Mid(Time, StartColon%, EndColon% - StartColon%)
TheMinute = Right(EndProduct$, 2)
End Function
Function TheSecond()
Rem Returns the seconds in the current time
Dim TheTime$
TheTime = Left(Time, Len(Time) - 3)
TheSecond = Right(TheTime, 2)
End Function
Function YahooName()
Rem Returns the top most pm name if there is one
On Error Resume Next
Dim imclass As Long
...
If InStr(1, TheText, " -- ") Then
Dim NoSpaces
Dim sName As String, sSplit As String
sName = TheText
sSplit = InStr(sName, " -- ")
NoSpaces = Left(sName, sSplit)
...
If InStr(1, TheText, " - ") Then
Dim NoSpaces1
Dim sName1 As String, sSplit1 As String
sName1 = TheText
sSplit1 = InStr(sName1, " - ")
NoSpaces1 = Left(sName1, sSplit1)
NoSpaces1 = Trim(NoSpaces1)
YahooName = NoSpaces1
End If
End Function
Sub ClosePm()
Rem Closes top most pm yahoo pm
Dim imclass As Long
imclass = FindWindow("imclass", vbNullString)
Call SendMessageLong(imclass, WM_CLOSE, 0&, 0&)
End Sub
Sub YChatSend(WhatToSay As String)
Rem Sends text to top most yahoo im
Dim imclass As Long
Dim richedit As Long
Dim button As Long
imclass = FindWindow("imclass", vbNullString)
richedit = FindWindowEx(imclass, 0&, "richedit", vbNullString)
Call SendMessageByString(richedit, WM_SETTEXT, 0&, WhatToSay)
imclass = FindWindow("imclass", vbNullString)
...
End Sub
Function Isimclass() As Boolean
Rem Finds the yahoo im window
Dim imclass As Long
imclass = FindWindow("imclass", vbNullString)
If imclass > 0 Then Isimclass = True: Exit Function
Isimclass = False
End Function
